...
resource "proxmox_virtual_environment_file" "gpu_hookscript" {
  count = var.use_gpu_hookscript ? 1 : 0
  ...
  source_raw {
    data = file("${path.module}/../../scripts/gpu/gpu-hookscript.sh")
...
resource "proxmox_virtual_environment_vm" "gpu_workers_hookscript" {
  count = var.use_gpu_hookscript ? var.gpu_worker_count : 0
  ...
  # Hookscript
  hookscript = proxmox_virtual_environment_file.gpu_hookscript[0].id
